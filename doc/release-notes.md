...
mnsc Coin version *version* is now available from:  <https://github.com/mnsavings/mnsavings/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/mnsavings/mnsavings/issues>
...
If you are running an older version, shut it down. Wait until it has completely shut down (which might take a few minutes for older versions), then run the installer (on Windows) or just copy over /Applications/MNSAVINGS-Qt (on Mac) or mnsavingsd/mnsavings-qt (on Linux).


Compatibility
==============

mnsc Coin is extensively tested on multiple operating systems using the Linux kernel, macOS 10.10+, and Windows 7 and later.

Microsoft ended support for Windows XP on [April 8th, 2014](https://www.microsoft.com/en-us/WindowsForBusiness/end-of-xp-support), No attempt is made to prevent installing or running the software on Windows XP, you can still do so at your own risk but be aware that there are known instabilities and issues. Please do not report issues about Windows XP to the issue tracker.

Apple released it's last Mountain Lion update August 13, 2015, and officially ended support on [December 14, 2015](http://news.fnal.gov/2015/10/mac-os-x-mountain-lion-10-8-end-of-life-december-14/). mnsc Coin software starting with v3.2.0 will no longer run on MacOS versions prior to Yosemite (10.10). Please do not report issues about MacOS versions prior to Yosemite to the issue tracker.

mnsc Coin should also work on most other Unix-like systems but is not frequently tested on them.
...
You can read more details about this extensive work in ([PR #954](https://github.com/mnsavings/MNSAVINGS/pull/954))
...
A brand new feature is being introduced with the release of v4.0.0: Cold Staking ([PR #955](https://github.com/mnsavings/MNSAVINGS/pull/955))! This feature allows a coin owner to keep coins in a "cold" (or locked) wallet whilst a "hot" wallet carries out the burden of staking those coins.
...
A full technical writup is available on the [MNSAVINGS Wiki](https://github.com/mnsavings/MNSAVINGS/wiki/ColdStaking), and an initial video showcase is available on [YouTube](https://www.youtube.com/watch?v=utxB5TzAeXc).
A brief guide to setup cold staking with GUI and RPC is available [here](https://github.com/random-zebra/MNSAVINGS-Wiki/blob/master/User-Documentation/Cold-Staking-HowTo.md).

<sup>1</sup> Spending cold stakes from HW wallets currently available only for Ledger devices via [PET4L](https://github.com/mnsavings/PET4L) tool.

Multi-Split Stake Splitting
-------------------

Stake splitting has received a makeover and now supports splitting to more than two (2) outputs. [PR #968](https://github.com/mnsavings/MNSAVINGS/pull/968) introduced the change, which is controlled by the wallet's `stakesplitthreshold` setting.

The default split threshold remains at 2000 MNSC, and can be adjusted in the GUI's Settings page, or via the RPC `setstakesplitthreshold` command.
...
### V1 zMNSC Spending (Public Spends Version 4)

Since the discovery of a critical exploit within the libzerocoin library in early 2019, remaining legacy v1 zMNSC have been un-spendable. We're happy to say that, once the new consensus rules are in effect, users will once again be able to spend their v1 zMNSC with public spends version 4 ([PR #936](https://github.com/mnsavings/MNSAVINGS/pull/936)).

As with the previous version 3 public spends introduced in core wallet version 3.3.0 (enabling the spending of v2 zMNSC), version 4 spends will also be public. A full technical writeup is available on the [MNSAVINGS Wiki](https://github.com/mnsavings/MNSAVINGS/wiki/CoinRandomnessSchnorrSignature).
...
[#PR1022](https://github.com/mnsavings/MNSAVINGS/pull/1022) defines Version 7 blocks, which remove the (now-unused) accumulator checkpoint from the block header. This results in an overall data reduction of ~256 bits from each block as well as the in-memory indexes.

### New Network Message Signatures

Layer 2 network messages (MN, Budget, Spork, etc) are now signed based on the hash of their **binary** content instead of their **string** representation ([#PR1024](https://github.com/mnsavings/MNSAVINGS/pull/1024)).

### New SPORKS

Two new SPORKS are introduced, `SPORK_17` ([#PR975](https://github.com/mnsavings/MNSAVINGS/pull/975)) and `SPORK_18` ([#PR995](https://github.com/mnsavings/MNSAVINGS/pull/995)).<br>
...
`SPORK_18` (off by default) is used to switch between Version 3 and [Version 4 Public Spends](#v1-zmnsc-spending-public-spends-version-4). When this spork is active, only version 4 spends are accepted by the network. When it's not, only version 3 spends are accepted.
...
- `delegated_balance` - MNSC balance held in P2CS contracts (delegated amount total).
- `cold_staking_balance` - MNSC balance held in cold staking addresses.
...
Returns a new MNSAVINGS cold staking address for receiving delegated cold stakes.

Arguments:
1. "account"        (string, optional) The account name for the address to be linked to. if not provided, the default account "" is used. It can also be set to the empty string "" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.

Result:
"mnsavingsaddress"    (string) The new mnsavings address
```
...
Arguments:
1. "stakingaddress"      (string, required) The mnsavings staking address to delegate.
2. "amount"              (numeric, required) The amount in MNSC to delegate for staking. eg 100
3. "owneraddress"        (string, optional) The mnsavings address corresponding to the key that will be able to spend the stake.
  ...
  "vout" : [              (array of json objects)
     {
       ...
       "scriptPubKey" : {          (json object)
         ...
         "addresses" : [           (json array of string)
           "mnsavingsaddress"        (string) mnsavings address
           ,...
         ]
       }
     }
...
Result:
amount              (numeric) The total amount in MNSC received for this account in P2CS contracts.
```
...
[
   ...
   "address": "xxx",  (string) MNSAVINGS address string
   }
  ...
]
```
...
For our linux users, in addition to the [Ubuntu PPA](https://launchpad.net/~mnsavings) repository, we are now offering a [Snap package](https://snapcraft.io/mnsavings-core) as quick way to install and update a MNSAVINGS wallet.
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/mnsavings-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
