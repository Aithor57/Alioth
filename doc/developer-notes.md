...
- [Development guidelines](#development-guidelines)
    - [General mnsc Coin](#general-mnsavings-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
mnsc Coin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play MNSCs (tMNSC)" on the test network, if you
...
mnsc Coin is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/mnsavings/mnsavings/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_mnsavings
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_mnsavings --log_level=test_suite
$ valgrind -v --leak-check=full src/mnsavingsd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_mnsavings.coverage/index.html`.
```
...
- BitcoinMiner : Generates MNSCs (if wallet is enabled).
...
However, in open source software such as mnsc Coin, where everyone uses
...
pay attention to for reviewers of mnsc Coin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: mnsc Coin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against mnsc Coin so that they can be integrated
...
file descriptors it will cause problems with MNSAVINGS's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:mnsavings/MNSAVINGS.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `mnsavings-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `mnsavings-cli` and the GUI debug console use this table to determine how to
