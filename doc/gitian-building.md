...
*Setup instructions for a gitian build of mnsc Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the MNSAVINGS
...
to the MNSAVINGS GitHub release page.
...
- [Building mnsc Coin](#building-mnsavings-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/mnsavings-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/mnsavings-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/mnsavings-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/mnsavings-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/mnsavings-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/mnsavings-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/mnsavings/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@mnsavings.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/mnsavings-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/mnsavings-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/mnsavings-osx-*-build.assert
```
